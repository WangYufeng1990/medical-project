...
| 实体 | 加密字段 | 原因 |
|------|---------|------|
| Patient | `name`, `ssn`, `phoneMobile`, `phoneHome`, `insuranceMemberId` | HIPAA Protected Health Information |
...
| 端点 | 输出 | 说明 |
|------|------|------|
| `GET /api/v1/fhir/metadata` | `CapabilityStatement` (JSON) | 声明 FHIR 版本(4.0.1)和支持的资源类型 |
...
```
 ...
 5. AesCryptoUtil           ← 加密算法和 static bridge 模式
 6. AesAttributeConverter   ← JPA Converter 如何透明加解密
 7. AuditLogAspect          ← AOP 如何拦截 @Auditable 方法
 8. AuditLogWriter          ← 异步写入 + 独立事务
 9. SecurityConfig          ← Spring Security + OAuth2 Resource Server
10. JwtClaimMapper          ← Okta claims → Spring Security 权限映射
11. PatientCaseService      ← FHIR Bundle 构建（FHIR 互操作）
12. BillService             ← 保险理赔状态机
13. DataInitializer         ← 种子数据覆盖所有模块
```
