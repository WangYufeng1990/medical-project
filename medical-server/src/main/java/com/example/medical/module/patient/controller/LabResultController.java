...
import com.example.medical.module.patient.dto.LoincCatalogVO;
...
public class LabResultController {
    ...
    public Result<List<LoincCatalogVO>> catalog() {
        return Result.ok(loincCatalogRepository.findAll()
                .stream().map(LoincCatalogVO::fromEntity).toList());
    }

    @GetMapping("/loinc/panel/{parentCode}")
    @PreAuthorize("hasAnyRole('ADMIN','DOCTOR')")
    public Result<List<LoincCatalogVO>> panel(@PathVariable String parentCode) {
